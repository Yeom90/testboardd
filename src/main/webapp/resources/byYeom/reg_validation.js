...
$('#empId').keyup(function () {
    ...
    if($('#empId').val() == ""){
        ...
        $('#empIdChecked').text("아이디는 영문 대소문자, 숫자 4~12자리로 입력해야 함.").css('color', 'red');
        isIdOk = false;
    }else{
        $('#empIdChecked').text("");
        isIdOk = true;
...
$('#empName').keyup(function () {
    ...
    if($('#empName').val() == ""){
        $('#empNameChecked').text("공백 입니다.").css('color', 'red');
        isNameOk = false;
    }else if(!nameValidation.test($('#empName').val())){
        $('#empNameChecked').text("사원명은 한글만 가능합니다.").css('color', 'red');
        isNameOk = false;
    }else{
        $('#empNameChecked').text("");
        isNameOk = true;
...
//내선번호 유효성 검사
$('#interphone').keyup(function () {
    isClickedInterphone = false; //중복검사전
    let interphoneValidation = /^[0-9]{4}$/; // 숫자 4글자(유효성검사)
    if($('#interphone').val() == ""){
        $('#interphoneChecked').text("공백 입니다..").css('color', 'red');
        isInterphoneOk = false;
    }else if(!interphoneValidation.test($('#interphone').val())){
        $('#interphoneChecked').text("아이디는 영문 대소문자, 숫자 4~12자리로 입력해야 함.").css('color', 'red');
        isInterphoneOk = false;
    }else{
        $('#interphoneChecked').text("");
        isInterphoneOk = true;
    }
});

//내선번호 중복 체크 로직
$('#checkInterphoneBtn').click(function () {
    if(isInterphoneOk){
        let interphone = $("#interphone").val();
        $.ajax({
            url : "/interphoneCheck",
            type : "POST",
            ...
            success: function (result) {
                if(result == "0"){
                    $('#interphoneChecked').text("사용 가능한 번호 입니다.").css('color', 'blue');
                    isClickedInterphone = true;
                }else{
                    $('#interphoneChecked').text("이미 있는 번호 입니다.").css('color', 'red');
